...
class GBNClient(UDPClient):
        ...
        with open(complete_path, 'wb') as file:
            ...
            while remaining_bytes > 0:
                ...
                try:
                    ...
                    logger.log_recv_pack_number(seq_num, args)
                except timeout:
        ...
        with open(complete_path, 'rb') as file:
            ...
            while last_acked_recv < expected_final_ack:
                if time.time() - global_timer > MAX_WAITING_TIME * 4:
                    logger.log_connection_failed()
                    return
                self.send_n_packets(last_acked_recv, GBN_WINDOW_SIZE, file, server_ip, port, args)
                ...
                while time.time() - last_ack_time < 1:
                    try:
                        ...
                        if recv_seq_num == last_acked_recv:
                            break  # Probably packet lost, send again
    ...
    def send_n_packets(self, initial, n, file: BufferedReader, server_ip, port, args):
        '''
        ...
        for seq_num in range(initial, initial + n):
            ...
            logger.log_packet_seq_number(seq_num, args)
